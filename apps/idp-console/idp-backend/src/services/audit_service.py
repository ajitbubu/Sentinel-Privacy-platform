"""Append-only audit logging. The audit_log table blocks UPDATE/DELETE by rule."""
import json
    ...
    db.execute(
        text("""
            INSERT INTO audit_log (entity_type, entity_id, action, actor_type, actor_id,
                                   ...
                                   reason, legal_basis)
            VALUES (:etype, :eid, :action, :atype, :aid,
                    CAST(:aip AS INET), CAST(:old AS JSONB), CAST(:new AS JSONB), :changed,
                    :reason, :basis)
        """),
        {
            "etype": entity_type, "eid": entity_id, "action": action,
            "atype": actor_type, "aid": actor_id, "aip": actor_ip,
